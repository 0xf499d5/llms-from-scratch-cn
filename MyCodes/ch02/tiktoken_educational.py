...
class SimpleBytePairEncoding:
    def __init__(self, *, pat_str: str, mergeable_ranks: dict[bytes, int]) -> None:
        ...
        self._decoder = {token: token_bytes for token_bytes, token in mergeable_ranks.items()}  # token id to token
    ...
    @staticmethod
    def train(training_data: str, vocab_size: int, pat_str: str, visualise: str | None = None):
...
def bpe_train(
    ...
    while len(ranks) < vocab_size:
        ...
        token = len(ranks)
        # Add the new token!
        ranks[token_bytes] = token  # 出现频率越高的pair，由他们合并后的token的id越小
